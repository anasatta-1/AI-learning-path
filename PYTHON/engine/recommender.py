...
def recommend_courses(student_id: int,
    ...
    #------Summer Training eligibility gate---------
    SUMMER_TRAINING_ID = 'AIEN300'
    YEAR3_CORE_IDS = {
        'CMPE343', 'CMPE351', 'EELE321', 'AIEN301',
        'CMPE326', 'CMPE332', 'AIEN302'
    }
    if SUMMER_TRAINING_ID in all_candidates:
        passed_year3 = sum(1 for cid in YEAR3_CORE_IDS if cid in passed_ids)
        if passed_year3 < len(YEAR3_CORE_IDS) * 0.70: #less than 5 of 7
            all_candidates = [c for c in all_candidates if c != SUMMER_TRAINING_ID]
    #-----------------------------------------------
